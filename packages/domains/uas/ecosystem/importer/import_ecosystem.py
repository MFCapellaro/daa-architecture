...
- import dealers when explicitly nested under "Dealers";
...
The importer records structural relationships explicitly observed
in the source:

    dealer -> representative

The brand remains contextual evidence attached to the relationship.
It is NOT interpreted as a node and no semantic relationship is invented.
...
def emit_relationships(
    ...
    """
    Emit observed structural relationships as YAML.
    """

    lines = [
        "# DRONSAIR Ecosystem Relationships",
        "#",
        "# Relationships observed directly in the source corpus.",
        "#",
        "# This file does not establish semantic identity.",
        "# The brand remains contextual evidence.",
        "",
        "relationships:",
    ]
    ...
    for relationship in relationships:
        ...
        rid = relationship_id(
            relationship["dealerId"],
            relationship["representativeId"],
            "represented_by",
        )

        lines += [
            f'  - id: {yaml_quote(rid)}',
            f'    source: {yaml_quote(relationship["dealerId"])}',
            "    type: represented_by",
            f'    target: {yaml_quote(relationship["representativeId"])}',
            "",
            "    context:",
            (
                f'      brand: '
                f'{yaml_quote(relationship["brand"])}'
            ...
            (
                f'      source: '
                f'{yaml_quote("ecosystem-source.txt")}'
            ),
            f'      line: {relationship["sourceLine"]}',
            (
                f'      section: '
                f'{yaml_quote(relationship["section"])}'
            ),
            "",
            "    status:",
